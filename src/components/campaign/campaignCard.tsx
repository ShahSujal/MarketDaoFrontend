...
import { getTokenQueryKey } from "wagmi/query";
...
const CampaignCard = ({ campaign }: Props) => {
  return (
    <div className=" w-[320px] h-[380px] my-4 rounded-xl mx-3 relative bg-gradient-to-br from-[#4342425b] to-[#050505bd]  items-center flex flex-col border-2 border-[#171717] backdrop-blur-lg z-10 ">
      ...
      />
      <div className=" flex flex-row justify-between items-center absolute right-3 top-1">
        <h1 className=" px-2 bg-black rounded-xl">{campaign.status}</h1>
      ...
      <div className=" w-full flex flex-col justify-center items-center">
        <div className=" flex w-full flex-row justify-between items-center px-3 mt-2">
          ...
          <div className=" text-sm flex flex-row justify-center items-center font-paps font-extralight text-gray-300">
           SYK Token {campaign.totalPrize}  
             <Image
          src={
            chainsWithLogo.find(
          ...
          height={30}
          alt=""
          className=" w-4 mx-2 h-4 rounded-full object-cover"
        />
          </div>
        </div>
